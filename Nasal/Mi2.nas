...
##                  Date : 05/05/2011                     ##
##          Trying to use it again in the Mi2             ##
############################################################

var RPM_arm=props.globals.getNode("/instrumentation/alerts/rpm",1);
var last_time = 0;
var start_timer=0;
var GPS = 0.0158333333;  ### avg cruise = 56 gph at about 70 knots
...
setprop("/Mi2/engines/engine[0]/mp-pressure",1);
setprop("/sim/model/Mi2/gps-visible",1);
...
var update_systems = func {
	...
	var engineTrottle = getprop("/controls/engines/engine/throttle"); #mp gauge based on throttle
     ...
     interpolate("/Mi2/engines/engine[0]/mp-pressure", (engineTrottle*0.44) + 0.45, 0.9);
        ...
        if(getprop("/engines/engine/amp-v") > 0){
            ...
            interpolate("/rotors/main/rpm", 2700 * throttle, 7);
            interpolate("/rotors/tail/rpm", 2700 * throttle, 7);
            }else{
            interpolate("/rotors/main/rpm", 0, 0.2);
            interpolate("/rotors/tail/rpm", 0, 0.2);
            }
            interpolate("/engines/engine/rpm", 2700 * throttle, 7);
        } else {
	...
	interpolate("oiltemp",0,20);
	  ...
	  interpolate("/engines/engine/rpm", 0, 7);
	  interpolate("/rotors/main/rpm", 0, 7);
	  interpolate("/rotors/tail/rpm", 0, 7);
      setprop("/Mi2/engines/engine[0]/mp-pressure",1);
	}
